import{_ as O}from"./preload-helper.s5mF4tiM.js";import{d as y,l as d,e as h}from"./index.lg0qkVDO.js";import{g as k}from"./scope.Dtq5cmHt.js";import"./index.DrQ6bw7J.js";import"./_commonjsHelpers.BosuxZz1.js";const C=`<CsoundSynthesizer>
<CsInstruments>
...
endin`;let l,e;async function S(n=""){await g(),n&&(n=`${n}`,await e?.evalCode(n))}const z=S,A=S,D=y("csound",(n,i)=>(n=n||"triangle",g(),i.onTrigger((t,s,a,m,p)=>{if(!e){d("[csound] not loaded yet","warning");return}s.ensureObjectValue();let{gain:o=.8}=s.value;o*=.2;const c=Math.round(h(s)),r=Object.entries({...s.value,freq:c}).flat().join("/"),v=p-a,u=`i ${[`"${n}"`,v,s.duration+0,c,o,`"${r}"`].join(" ")}`;e.inputMessage(u)})));function $(n,i){const[t]=i;if(n==="message"&&(["[commit: HEAD]"].includes(t)||t.startsWith("--Csound version")||t.startsWith("libsndfile")||t.startsWith("sr =")||t.startsWith("0dBFS")||t.startsWith("audio buffered")||t.startsWith("writing")||t.startsWith("SECTION 1:")))return;let s="info";t.startsWith("error:")&&(s="error"),d(`[csound] ${t||""}`,s)}async function x(){if(window.__csound__)return d("[load] Using external Csound","warning"),e=window.__csound__,e;{const{Csound:n}=await O(async()=>{const{Csound:i}=await import("./csound.rGqV05_F.js");return{Csound:i}},[]);return e=await n({audioContext:k()}),e.removeAllListeners("message"),["message"].forEach(i=>e.on(i,(...t)=>$(i,t))),await e.setOption("-m0d"),await e.setOption("--sample-accurate"),await e.setOption("-odac"),await e.compileCsdText(C),await e.compileOrc(q),await e.start(),e}}async function g(){return l=l||x(),l}let f={};async function I(n){if(await g(),typeof n!="string")throw new Error("loadOrc: expected url string");if(n.startsWith("github:")){const[i,t]=n.split("github:");n=`https://raw.githubusercontent.com/${t}`}f[n]||(f[n]=fetch(n).then(i=>i.text()).then(i=>e.compileOrc(i))),await f[n]}const L=y("csoundm",(n,i)=>{let t=n;return typeof n=="string"&&(t=`"${n}"`),g(),i.onTrigger((s,a)=>{if(!e){d("[csound] not loaded yet","warning");return}if(typeof a.value!="object")throw new Error("csound only support objects as hap values");const m=s-k().currentTime,p=a.duration.valueOf()+0,o=h(a);let{gain:c=1,velocity:r=.9}=a.value;r=c*r;const u=(Math.log(o/261.62558)/Math.log(2)+8)*12-36,b=127*r;Object.entries({...a.value,frequency:o}).flat().join("/");const w=`i ${t} ${m} ${p} ${u} ${b}`;console.log("[csoundm]:",w),e.inputMessage(w)})});export{D as csound,L as csoundm,S as loadCSound,A as loadCsound,I as loadOrc,z as loadcsound};
//# sourceMappingURL=index.Bzrn0ZNF.js.map
