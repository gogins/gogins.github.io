import{j as a}from"./jsx-runtime.XGodtQ4D.js";import{u as F,$ as v,c as w,d as C,e as D,f as B,p as x,g as k,s as G,h as g,j as h,k as E,m as S}from"./useClient.2n2CwdCY.js";import{r as f}from"./index.Nkt9jjly.js";function j(...s){return s.filter(Boolean).join(" ")}function _({value:s,onChange:n,items:e}){return a.jsx("div",{className:"flex max-w-lg",children:Object.entries(e).map(([o,c],d,p)=>a.jsx("button",{onClick:()=>n(o),className:j("px-2 border-b h-8 whitespace-nowrap",s===o?"border-foreground":"border-transparent"),children:c.toLowerCase()},o))})}const N=["by","url","genre","license"];function R(s){const n=/\/\*([\s\S]*?)\*\/|\/\/(.*)$/gm,e=[...s.matchAll(n)].map(p=>(p[1]||p[2]||"").trim()),o={},[c,d]=(e[0]||"").split('"');c.trim()===""&&d!==void 0&&(o.title=d);for(const p of e){const m=p.split("@").slice(1);for(const l of m){let[i,r]=l.split(/ (.*)/s);if(i=i.trim(),r=(r||"").replaceAll(/ +/g," ").trim(),N.includes(i)){const t=r.split(/[,\n]/).map(b=>b.trim()).filter(b=>b!=="");o[i]=i in o?o[i].concat(t):t}else r=r.replaceAll(/\s+/g," "),o[i]=i in o?o[i]+" "+r:r}}return o}const T=`// Koji Kondo - Swimming (Super Mario World)
stack(
  seq(
    "~",
    "~",
...
.pianoroll({fold:1})`,q=`// Koji Kondo - Princess Zelda's Rescue
stack(
  // melody
...
note(
  ...
  .gain(.5)
  .room(.5)
  .lpa(.125).lpenv(-2).v("8:.125").fanchor(.25)`,V=`// "Hyperpop"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
// strudel disable-highlighting`,Z=`// "Festival of fingers 3"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
stack(
    ...
    .delaytime(".16 | .33")
    .delayfeedback(".6 | .8")
  ).sometimes(x=>x.speed("-1"))`,ae=`// "Orbit"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
stack(
    ...
    .delayfeedback(.7)
    .orbit(2)
  ).sometimes(x=>x.speed("-1"))`,oe=`// "Belldub"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
).reset("<x@7 x(5,8,-1)>")`,re=`// "Jux und tollerei"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
.note()
//.pianoroll()
.csound('CoolSynth')`,pe=`// "Lounge sponge"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
...
`,me=Object.freeze(Object.defineProperty({__proto__:null,amensister:ie,arpoon:be,barryHarris:O,bassFuge:X,belldub:oe,blippyRhodes:K,caverave:M,chop:ee,csoundDemo:de,delay:se,dinofunk:te,echoPiano:z,festivalOfFingers:H,festivalOfFingers3:Z,flatrave:ce,giantSteps:P,goodTimes:$,holyflute:le,hyperpop:V,juxUndTollerei:re,loungeSponge:pe,meltingsubmarine:Q,orbit:ae,randomBells:I,sampleDemo:ne,sampleDrums:Y,sml1:J,swimming:T,undergroundPlumber:W,waa2:U,wavyKalimba:L,zeldasRescue:q},Symbol.toStringTag,{value:"Module"}));Object.fromEntries(Object.entries(me).map(([s,n],e)=>[e,{id:e,code:n,collection:"Stock Examples"}]));const he=()=>{const s=F(v),n=F(w),e=f.useMemo(()=>{const c=new Map;return c.set(C.featured,s),c.set(C.public,n),c},[s,n]);return{patterns:f.useMemo(()=>Object.assign({},...e.values()),[e]),collections:e}};function ue(...s){return s.filter(Boolean).join(" ")}function ge({pattern:s}){const n=f.useMemo(()=>R(s.code),[s]);let e=n.title;if(e==null){const o=new Date(s.created_at);isNaN(o)||(e=o.toLocaleDateString())}return e==null&&(e=s.hash),e==null&&(e="unnamed"),a.jsx(a.Fragment,{children:`${s.id}: ${e} by ${Array.isArray(n.by)?n.by.join(","):"Anonymous"}`})}function fe({showOutline:s,onClick:n,pattern:e,showHiglight:o}){return a.jsx("a",{className:ue("mr-4 hover:opacity-50 cursor-pointer block",s&&"outline outline-1",o&&"bg-selection"),onClick:n,children:a.jsx(ge,{pattern:e})})}function y({patterns:s,activePattern:n,onClick:e,started:o}){const c=B(),p=x(c).id;return a.jsx("div",{className:"font-mono text-sm",children:Object.values(s).reverse().map(m=>{const l=m.id;return a.jsx(fe,{pattern:m,showHiglight:l===p,showOutline:l===n&&o,onClick:()=>e(l)},l)})})}function u({children:s,onClick:n,label:e,labelIsHidden:o}){return a.jsxs("button",{className:"hover:opacity-50",onClick:n,title:e,children:[o!==!0&&e,s]})}function Be({context:s}){const n=D(),e=B(),o=x(e),{userPatterns:c,patternFilter:d}=k(),m=he().collections,l=(t,b=!1)=>{s.handleUpdate(t,b)},i=o?.id,r=!window.parent?.location.pathname.includes("oodles");return a.jsxs("div",{className:"px-4 w-full dark:text-white text-stone-900 space-y-2 pb-4 flex flex-col overflow-hidden max-h-full",children:[a.jsx(_,{value:d,onChange:t=>G.setKey("patternFilter",t),items:g}),d===g.user&&a.jsx("div",{children:a.jsxs("div",{className:"pr-4 space-x-4 border-b border-foreground flex max-w-full overflow-x-auto",children:[a.jsx(u,{label:"new",onClick:()=>{const{data:t}=h.createAndAddToDB();l(t)}}),a.jsx(u,{label:"duplicate",onClick:()=>{const{data:t}=h.duplicate(o);l(t)}}),a.jsx(u,{label:"delete",onClick:()=>{const{data:t}=h.delete(i);l({...t,collection:h.collection})}}),a.jsxs("label",{className:"hover:opacity-50 cursor-pointer",children:[a.jsx("input",{style:{display:"none"},type:"file",multiple:!0,accept:"text/plain,application/json",onChange:t=>E(t.target.files)}),"import"]}),a.jsx(u,{label:"export",onClick:S}),a.jsx(u,{label:"delete-all",onClick:()=>{const{data:t}=h.clearAll();l(t)}})]})}),a.jsxs("section",{className:"flex overflow-y-scroll max-h-full flex-col",children:[d===g.user&&a.jsx(y,{onClick:t=>l({...c[t],collection:h.collection},r),patterns:c,started:s.started,activePattern:n,viewingPatternID:i}),d!==g.user&&Array.from(m.keys()).map(t=>{const b=m.get(t);return a.jsxs("section",{className:"py-2",children:[a.jsx("h2",{className:"text-xl mb-2",children:t}),a.jsx("div",{className:"font-mono text-sm",children:a.jsx(y,{onClick:A=>l({...b[A],collection:t},r),started:s.started,patterns:b,activePattern:n})})]},t)})]})]})}export{_ as B,ge as P,Be as a,j as c};
