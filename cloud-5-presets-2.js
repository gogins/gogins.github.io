...
class Cloud5Piece extends HTMLElement {
    ...
    */
    ...
    set control_parameters_addon(parameters_) {
        ...
        TODO: This is bullshit. Probably need to recreate and reattach dat.gui here.
        this.gui.params.load = Object.assign(this.gui.params.load, parameters_);
    ...
    <ul class="menu">
        ...
        dat_gui_parameters = Object.assign(dat_gui_parameters, this.control_parameters_addon);
    ...
    copy_parameters() {
        ...
        this?.csound?.Message("Copied _current_ control parameters to system clipboard.\n")
    }
    ...
    render = async function (is_offline) {
        ...
        // Send _selected_ dat.gui parameter values to Csound 
        // before actually performing.
        this.send_parameters(this.active_control_parameters);
        ...
        if (is_offline == false) {
            ...
            if (typeof strudel_view !== 'undefined') {
                if (strudel_view !== null) {
                    ...
                    strudel_view?.setParameters(this.active_control_parameters);
    ...
    save_preset(preset_name, preset_values) {
        if (!this.gui.load.remembered) {
            this.gui.load.remembered = {};
        }
        this.gui.load.remembered[preset_name] = preset_values;
        // const option = document.createElement('option');
        // option.innerHTML = preset_name;
        // option.value = preset_name;
        // this.gui.__preset_select.appendChild(option);
    ...
    /**
     * Updates the dat.gui menu with the values of the parameters addon. 
     */
    update_menu() {
        ///this.gui.remember(this.#control_parameters_addon);
        this.gui.remember();
        if (this.#control_parameters_addon.remembered) {
            Object.entries(this.#control_parameters_addon.remembered).forEach(([preset_name, preset_values]) => {
                console.log(`Key: ${preset_name}, Value: ${preset_values}`);
                this.save_preset(preset_name, preset_values)
            });
        } else {
            this.save_preset('Default', this.#control_parameters_addon);
        }
     ...
     * @param {string} name_ An optional visible label for the slider.
     */
    menu_slider_addon(gui_folder, token, minimum, maximum, step, name_) {
        const on_parameter_change = ((value) => {
            this.gk_update(token, value);
        });
        if (!gui_folder.__controllers.some(ctrl => ctrl.property === token)) {
            if (name_) {
                gui_folder.add(this.active_control_parameters, token, minimum, maximum, step).name(name_).listen().onChange(on_parameter_change);
            } else {
                gui_folder.add(this.active_control_parameters, token, minimum, maximum, step).listen().onChange(on_parameter_change);
            }
...
class Cloud5Strudel extends HTMLElement {
    ...
    set control_parameters_addon(parameters_) {
        this.#control_parameters_addon = parameters_;
        Object.assign(this.gui.load, parameters_);
        globalThis.parameters = parameters_;
        // Reconstruct the element.
        this.connectedCallback();
        this.gui.remember();
