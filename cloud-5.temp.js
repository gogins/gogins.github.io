...
class Cloud5Piece extends HTMLElement {
  ...
  */
  ...
  render = async function (is_offline) {
    ...
    // parameter values.
    for (const [name, value] of Object.entries(this.control_parameters_addon)) {
   ...
   * (from ChatGPT).
   */
  update_parameters_in_csd(csdText, variables) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inInstrBlock) {
            lines[i] = lines[i].replace(
                /^(gk_[A-Za-z0-9_]+)\s+init\s+(-?\d+(\.\d+)?)$/i,
                (match, varName, oldValue) => {
                    let newValue = variables[varName];  // Explicitly retrieve from `variables`
                    if (newValue !== undefined) {
                        return `${varName} init ${newValue}`;
                    }
                    return match;  // Keep original if no new value exists
                }
            );
        }
    }
